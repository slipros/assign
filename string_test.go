...
// TestString_Successfully tests successful scenarios of converting and setting string values.
func TestString_Successfully(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "string to *string", field: "PtrField", input: "hello", expected: ptrString("hello")},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Value the value
			err := String(field, tc.input)
			require.NoError(t, err, "String should not return error")
...
// TestString_Failure tests scenarios where String should return an error.
func TestString_Failure(t *testing.T) {
	// Create test structure
	type TestStruct struct {
		IntField             int
		UintField            uint
	...
	tests := []struct {
		...
		// Unsupported map types
		{
			...
			errCheck: func(err error) bool {
				return errors.Is(errors.Cause(err), ErrNotSupported)
			},
		},
		{
...
// TestString_UnexportedField tests that String returns an error when trying to set an unexported field
func TestString_UnexportedField(t *testing.T) {
	...
	err := String(field, "test value")

	// Check error
	assert.Error(t, err, "String should return error for unexported field")
}
...
// BenchmarkString benchmarks the String function with various types.
	...
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				_ = String(field, bm.value)
			}
		})
	}
}

// BenchmarkString_SameField benchmarks String with reuse of the same field.
// This is to test the performance when repeatedly setting values to the same field.
func BenchmarkString_SameField(b *testing.B) {
	type TestStruct struct {
		StringField  string
		IntField     int
		Float64Field float64
...
// BenchmarkString_SuccessVsFailure compares the performance of successful
// and failing String calls.
func BenchmarkString_SuccessVsFailure(b *testing.B) {
	type TestStruct struct {
	...
	b.Run("Failure_InvalidValue", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			_ = String(field, "letters")
		}
	})
}

// BenchmarkSetTimeString compares the performance of time parsing
// with different formats.
func BenchmarkSetTimeString(b *testing.B) {
	benchmarks := []struct {
		name   string
		format string
		value  string
...
// TestToSliceString_EdgeCases tests various edge cases and boundary conditions
// that could cause issues with the type safety fixes.
func TestToSliceString_EdgeCases(t *testing.T) {
	tests := []struct {
		name        string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				assert.Error(t, err, "String should return error for input: %s", tc.input)
			...
			require.NoError(t, err, "String should succeed for input: %s", tc.input)
...
// TestToSliceString_UnsupportedTypes tests that unsupported slice types
// return appropriate errors instead of panicking.
func TestToSliceString_UnsupportedTypes(t *testing.T) {
	tests := []struct {
		name       string
		setupField func() reflect.Value
		input      string
...
// TestToSliceString_MemoryLeaks tests for potential memory leaks
// in slice allocation and management.
func TestToSliceString_MemoryLeaks(t *testing.T) {
...
// TestToSliceString_ConcurrentAccess tests concurrent access to slice conversion
// to ensure thread safety of the type conversion process.
func TestToSliceString_ConcurrentAccess(t *testing.T) {
...
// TestToSliceString_TypeSafety_StringSlice tests the critical type safety fix
...
func TestToSliceString_TypeSafety_StringSlice(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		expected []string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Call setSliceString (through String which routes to it)
			err := String(field, tc.input)
			require.NoError(t, err, "String should succeed for input: %s", tc.input)
...
// TestToSliceString_TypeSafety_NumericSlice tests the critical type safety fix
// for numeric slice conversion that was implemented in lines 274-293.
func TestToSliceString_TypeSafety_NumericSlice(t *testing.T) {
	tests := []struct {
		name        string
		setupField  func() reflect.Value
		input       string
...
// TestToSliceString_TypeSafety_BoolSlice tests the critical type safety fix
// for bool slice conversion.
func TestToSliceString_TypeSafety_BoolSlice(t *testing.T) {
	tests := []struct {
		name        string
		input       string
		expected    []bool
...
// TestToSliceString_TypeSafety_SingleElement tests the critical type safety fix
// for single element conversion that was implemented in lines 303-306.
func TestToSliceString_TypeSafety_SingleElement(t *testing.T) {
	tests := []struct {
		name       string
		setupField func() reflect.Value
		input      string
...
// TestToSliceString_PanicPrevention tests that the type safety fixes
// prevent panics that could occur with the old implementation.
func TestToSliceString_PanicPrevention(t *testing.T) {
	tests := []struct {
		name       string
		setupField func() reflect.Value
		input      string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NoError(t, err, "String should succeed without panic: %s", tc.testDesc)
		})
	}
}

// TestToIntString_OverflowFix tests the integer overflow bug fix in setIntString function.
...
func TestToIntString_OverflowFix(t *testing.T) {
...
// TestToIntString_OptimizationPath tests that the optimization path (≤3 digits)
// works correctly and properly validates overflow conditions.
func TestToIntString_OptimizationPath(t *testing.T) {
	type TestStruct struct {
		Int8Field  int8
		Int16Field int16
		Int32Field int32
...
// BenchmarkSetIntString_OverflowFix tests that the overflow fix doesn't
// significantly impact performance for common cases.
func BenchmarkSetIntString_OverflowFix(b *testing.B) {
	type TestStruct struct {
		Int8Field  int8
		Int16Field int16
		Int32Field int32
	...
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				_ = String(field, bm.input)
			}
		})
	}
}

// TestToIntString_EdgeCases tests edge cases that could trigger the overflow bug.
func TestToIntString_EdgeCases(t *testing.T) {
	type TestStruct struct {
		Int8Field  int8
		Int16Field int16
		Int32Field int32
