...
func TestSliceString_Successfully(t *testing.T) {
	tests := []struct {
		name     string
		setup    func() (reflect.Value, []string)
		expected any
		options  []SliceOptionFunc
	}{
		{
		...
		},
		{
			...
			expected: "tag1|tag2|tag3",
			options:  []SliceOptionFunc{WithSeparator("|")},
		},
		{
			name: "string slice to []string",
			setup: func() (reflect.Value, []string) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := SliceString(field, values, tt.options...)
...
func TestSliceString_Failure(t *testing.T) {
	tests := []struct {
		name     string
		setup    func() (reflect.Value, []string)
		errorMsg string
...
// BenchmarkSliceString_StringFromString benchmarks converting a string slice to a string
func BenchmarkSliceString_StringFromString(b *testing.B) {
	var s struct {
		Tags string
	}
	...
	for i := 0; i < b.N; i++ {
		_ = SliceString(field, values)
	}
}

// BenchmarkSliceString_StringSliceString benchmarks converting a string slice to a []string
func BenchmarkSliceString_StringSliceString(b *testing.B) {
	var s struct {
		Tags []string
	}
	field := reflect.ValueOf(&s).Elem().FieldByName("Tags")
...
// BenchmarkSliceString_StringToSliceInt benchmarks converting a string slice to an []int
func BenchmarkSliceString_StringToSliceInt(b *testing.B) {
	var s struct {
		IDs []int
	}
...
// BenchmarkSliceString_WithSeparator benchmarks using a custom separator
func BenchmarkSliceString_WithSeparator(b *testing.B) {
	var s struct {
		Tags string
	}
	field := reflect.ValueOf(&s).Elem().FieldByName("Tags")
	...
	for i := 0; i < b.N; i++ {
		_ = SliceString(field, values, WithSeparator("|"))
	}
}
